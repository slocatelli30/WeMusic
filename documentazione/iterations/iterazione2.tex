...
Dopo aver analizzato nel dettaglio i casi d'uso, si procede con una modellazione più dettagliata 
tramite UML Component Diagram ed infine si analizzano le API fornite dal sistema. 
\newpage
...
Nel back-end sono stati implementati gli endpoint elencati nella seguente tabella. 
Questi endpoint costituiscono le fondamenta per il funzionamento di base della comunicazione tra l'utente e il sistema.
La tabella mostra il metodo, l'endpoint e la funzione dell'API corrispondente.
Questi endpoint consentono di interagire con il sistema per ottenere informazioni sugli 
utenti, crearne di nuovi, aggiornare i loro dettagli e eliminarli. Sono elementi chiave per 
la comunicazione e l'interazione tra l'utente e il back-end del sistema.
I campi in tabella sono elencati di seguito:
\begin{itemize}
    \item \textbf{Metodo:} indica il tipo di operazione che può essere eseguita 
        sull'endpoint specificato; quelli comuni e tradizionali includono i metodi HTTP (GET, POST, PUT e DELETE), 
        ognuno dei quali con uno scopo ben specifico. Nel contesto specifico del presente sistema, sviluppato con Django, 
        le logiche di backend che gestiscono le operazioni in questione hanno una numenclatura differente.
    \item \textbf{Endpoint:} rappresenta l'URL specifico a cui è possibile accedere per eseguire un'operazione 
        sulla risorsa corrispondente; essi sono parte dell'URL che segue il nome del dominio o l'indirizzo 
        IP del server. 
    \item \textbf{Funzione:} descrive brevemente l'obiettivo o la finalità dell'endpoint, cioè indica ciò 
        che l'endpoint fa o quale operazione esegue quando viene richiamato.
\end{itemize}
\begin{table} [h!]
\begin{center}
